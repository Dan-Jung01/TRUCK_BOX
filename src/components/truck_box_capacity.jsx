...
import { Container, Typography, Grid, TextField, Card, CardContent, Table, TableHead, TableRow, TableCell, TableBody, Button, Stack } from '@mui/material';
...
export default function TruckLoadCalculator() {
  ...
  return (
    <Container maxWidth="md" sx={{ mt: 5, backgroundColor: '#f5f7fa', p: 4, borderRadius: 3 }}>
      <Typography variant="h4" align="center" gutterBottom fontWeight={600} color="primary.main">
        트럭 적재 가능 박스 수 계산기
      </Typography>
        ...
        <CardContent>
          ...
          <Table>
            <TableHead sx={{ backgroundColor: '#f1f8e9' }}>
              <TableRow>
                <TableCell sx={{ fontWeight: 600 }}>트럭 종류</TableCell>
                <TableCell align="center" sx={{ fontWeight: 600 }}>내부 크기 (가로×세로×높이, mm)</TableCell>
                <TableCell align="right" sx={{ fontWeight: 600 }}>적재 가능 박스 수</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {trucks.map((truck) => (
                <TableRow key={truck.name}>
                  ...
                  <TableCell align="center">{`${truck.width} × ${truck.length} × ${truck.height}`}</TableCell>
                  <TableCell align="right">{calculateBoxes(truck)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
